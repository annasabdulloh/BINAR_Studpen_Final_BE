...
const sendNotify = async (userId, msg)=>{
  ...
  axios.get(`https://binarfinalsocketserver-production-f1fe.up.railway.app/set-notify/${notify.id}`)
}
  ...
  for (let i = 0; i < allTrxPending.length; i++) {
    ...
    if (res.data.status_code == 200) {
      ...
      if(trxs.length == 0){
        ...
        axios.get(`https://binarfinalsocketserver-production-f1fe.up.railway.app/set-notify/${trxs[0].id}`)
      }
    }
  }
